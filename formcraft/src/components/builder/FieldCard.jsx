...
export default function FieldCard({ field, isSelected, onSelect, onRemove }) {
  ...
  return (
    <article
      ...
      tabIndex={0}
      className={[
        "group mx-0 mb-2 flex cursor-pointer gap-2 rounded-lg border bg-surface p-3 shadow-sm transition-all hover:border-border-strong hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 sm:gap-3 sm:p-4",
        isSelected ? "border-brand-300 ring-2 ring-brand-500" : "border-border",
      ].join(" ")}
    >
      <button
        type="button"
        ...
        className="mt-0.5 flex h-8 w-8 shrink-0 touch-none cursor-grab items-center justify-center rounded-md text-text-disabled opacity-100 transition-colors hover:bg-surface-overlay hover:text-text-muted focus-visible:outline-none "
